...
function SpmSender (spmToken, processMetricsInterval, metricsApiEndpoint) {
  ...
  this.tid = null
  var apiEndpoint = (metricsApiEndpoint || process.env.SPM_RECEIVER_URL || config.get('spmSenderBulkInsertUrl') || defaultSpmSenderUrl)
  var metricsApiEndpointVerified = apiEndpoint
  ...
  this.sendEvent('server-info',
    ...
    function (err, result) {
      if (err) {
        ...
        logger.debug('SPM client info event:' + JSON.stringify(serverInfo) + ' ' + result.body)
      }
    })
}
